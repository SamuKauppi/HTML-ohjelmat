...
function createArticle(title, caption, desc, image){
  ...
  arcticle.append(h2)
  arcticle.append(figure);
  arcticle.append(p);
  figure.append(img);
  figure.append(fig);

  arcticle.addEventListener('click', function(){onclick(image)});
}

function onclick(image){
  let dia = document.querySelector('dialog');
  dia.lastElementChild.src = image.large
  dia.lastElementChild.alt = image.medium
  dia.showModal();  
}

for(let i = 0; i < picArray.length; i++){
  createArticle(picArray[i].title, picArray[i].caption, picArray[i].description, picArray[i].image);
}
let dia = document.querySelector('dialog');
